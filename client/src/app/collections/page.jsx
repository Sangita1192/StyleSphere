...
export default function Category() {
  ...
  //calculate the index range for products per page
  const indexOfLastProduct = currentPage * productsPerPage;
  const indexOfFirstProduct = indexOfLastProduct - productsPerPage;
  const currentProducts = allProd.slice(indexOfFirstProduct, indexOfLastProduct);

  // Change page handler
  const paginate = (pageNumber) => setCurrentPage(pageNumber);

  // Total pages
  const totalPages = Math.ceil(allProd.length / productsPerPage);
            ...
            <div className='text-center mt-10'>
              ...
              <div className="flex justify-center gap-2">
                <button 
                  ...
                  className="border-2 py-2 px-4 font-medium text-black disabled:opacity-50"
                >
                  Prev
                </button>
                {Array.from({ length: totalPages }).map((_, index) => (
                  <button 
                    ...
                    className={`border-2 py-2 px-4 font-medium text-black ${currentPage === index + 1 ? "bg-black text-white" : ""}`}
                  >
                    {index + 1}
                  </button>
                ))}
